...
public class Request extends Message {
	...
	/**
	 * Used when issuing a preflight CORS request to let the origin server knows
	 * what method the client is willing to send in future request to this resource.
	 */
	...
	/**
	 * Asks the server connector to immediately commit the given response associated
	 * to this request, making it ready to be sent back to the client. Note that all
	 * server connectors don't necessarily support this feature.
	 * 
	 ...
	 * request to this resource. Used when issuing a preflight CORS request to let
	 * the origin server knows what headers will be sent later.<br>
	 * Note that when used with HTTP connectors, this property maps to the
	 * "Access-Control-Allow-Headers" header.
	 * 
	 * @return The headers the client is willing to send in future request to this
	 *         resource. Useful for CORS support.
	 */
	 ...
	 * resource. Used when issuing a preflight CORS request to let the origin server
	 * knows what method will be sent later.<br>
	 * Note that when used with HTTP connectors, this property maps to the
	 * "Access-Control-Request-Method" header.
	 * 
	 * @return The method the client is willing to send in future request to this
	 *         resource. Useful for CORS support.
	 */
	...
	/**
	 * Returns the client-specific information. Creates a new instance if no one has
	 * been set.
	 * 
	...
	/**
	 * Returns the modifiable series of cookies provided by the client. Creates a
	 * new instance if no one has been set.<br>
	 * <br>
	 * Note that when used with HTTP connectors, this property maps to the "Cookie"
	 * header.
	 * 
	...
	public Protocol getProtocol() {
		...
		if ((result == null) && (getResourceRef() != null)) {
			...
			if (result == null) {
				result = (getResourceRef().getBaseRef() != null) ? getResourceRef().getBaseRef().getSchemeProtocol()
						: null;
			}
		}
	...
	/**
	 * Returns the authentication response sent by a client to a proxy. Note that
	...
	/**
	 * Returns the ranges to return from the target resource's representation. Note
	...
	/**
	 * Returns the referrer reference if available. Note that when used with HTTP
	...
	/**
	 * Returns the reference of the target resource. This reference is especially
	 * important during routing, dispatching and resource finding. During such
	 * processing, its base reference is constantly updated to reflect the reference
	 * of the parent Restlet or resource and the remaining part of the URI that must
	 * be routed or analyzed.
	 * 
	 * If you need to get the URI reference originally requested by the client, then
	 * you should use the {@link #getOriginalRef()} method instead. Also, note that
	 * beside the update of its base property, the resource reference can be
	 * modified during the request processing.
	 * 
	 * For example, the {@link org.restlet.service.TunnelService} associated to an
	 * application can extract some special extensions or query parameters and
	 * replace them by semantically equivalent properties on the request object.
	 * Therefore, the resource reference can become different from the original
	 * reference.
	 * 
	 * Finally, when sending out requests via a dispatcher such as
	 * {@link Context#getClientDispatcher()} or
	 * {@link Context#getServerDispatcher()}, if the reference contains URI template
	 * variables, those variables are automatically resolved using the request's
	 * attributes.
	 * 
	...
	/**
	 * Indicates if the request is asynchronous. The test consist in verifying that
	 * the {@link #getOnResponse()} method returns a callback object.
	 * 
	...
	/**
	 * Indicates if a content is available and can be sent. Several conditions must
	 * be met: the method must allow the sending of content, the content must exists
	 * and have some available data.
	 * 
	...
	/**
	 * Indicates if the request is synchronous. The test consist in verifying that
	 * the {@link #getOnResponse()} method returns null.
	 * 
	 * @return True if the request is synchronous.
	 */
	 ...
	 * this resource. Used when issuing a preflight CORS request to let the origin
	 * server knows what headers will be sent later.<br>
	 * Note that when used with HTTP connectors, this property maps to the
	 * "Access-Control-Request-Method" header.
	 * 
	 * @param accessControlRequestHeaders The set of headers the client is willing
	 *                                    to send in future request to this
	 *                                    resource. Useful for CORS support.
	 */
	 ...
	 * "Access-Control-Request-Method" header.
	 * 
	 * @param accessControlRequestMethod The method the client is willing to send in
	 *                                   future request to this resource. Useful for
	 *                                   CORS support.
	 */
	...
	/**
	 * Sets the authentication response sent by a client to an origin server. Note
	...
	/**
	 * Sets the modifiable series of cookies provided by the client. Note that when
	 * used with HTTP connectors, this property maps to the "Cookie" header. This
	 * method clears the current series and adds all entries in the parameter
	 * series.
	 * 
	...
	/**
	 * Sets the authentication response sent by a client to a proxy. Note that when
	 * used with HTTP connectors, this property maps to the "Proxy-Authorization"
	 * header.
	 * 
	 * @param challengeResponse The authentication response sent by a client to a
	 *                          proxy.
	 */
	 ...
	 * representation. Note that when used with HTTP connectors, this property maps
	 * to the "Range" header. This method clears the current list and adds all
	...
	/**
	 * Sets the target resource reference. If the reference is relative, it will be
	...
	public String toString() {
		return ((getMethod() == null) ? "" : getMethod().toString()) + " "
				+ ((getResourceRef() == null) ? "" : getResourceRef().toString()) + " "
				+ ((getProtocol() == null) ? ""
						: (getProtocol().getName()
								+ ((getProtocol().getVersion() == null) ? "" : "/" + getProtocol().getVersion())));
	}
