...
public class Response extends Message {
	 ...
	 * prefer obtaining the current context using methods such as
	...
	/**
	 * When used as part of a response to a preflight CORS request, this lists the
	 * headers allowed by the actual request on the current resource.
	 */
	private volatile Set<String> accessControlAllowHeaders;

	/**
	 * When used as part of a response to a preflight CORS request, this lists the
	 * methods allowed by the actual request on the current resource.
	 */
	 ...
	 * server allows for the requested resource. Use "*" as a wildcard character.
	 */
	...
	/**
	 * When used in the context of CORS support, it indicates how long the results
	 * of a preflight request can be cached in a preflight result cache.
	 */
	private volatile int accessControlMaxAge;

	/**
	 * Estimated amount of time since a response was generated or revalidated by the
	 * origin server.
	 */
	...
	/**
	 * The authentication information sent by an origin server to a client in the
	 * case of a successful authentication attempt.
	 */
	...
	/**
	 * Asks the server connector to immediately commit the given response, making it
	 * ready to be sent back to the client. Note that all server connectors don't
	 * necessarily support this feature.<br>
	 * <br>
	 * When the response is in autoCommit mode (see related property), then calling
	 * this method isn't necessary. Also, be aware that committing the response
	 * doesn't necessarily means that is will be immediately be written on the
	 * network as some buffering can occurs. If you want to ensure that response
	 * buffers are flushed,<br>
	 * <br>
	...
	/**
	 * Asks the server connector to immediately flush the network buffers. Note that
	 * this calls back {@link Request#flushBuffers()} on the parent request which
	 * holds the link with the underlying network connection.
	 * 
	 * @throws IOException
	 */
	@Override
	 ...
	 * allows for the requested resource. Use "*" as a wildcard character.<br>
	 * Note that when used with HTTP connectors, this property maps to the
	 ...
	 * preflight result cache.<br>
	 * In case of a negative value, the results of a preflight request is not meant
	 * to be cached.<br>
	 * Note that when used with HTTP connectors, this property maps to the
	 * "Access-Control-Max-Age" header.
	 * 
	 * @return Indicates how long the results of a preflight request can be cached
	 ...
	 * revalidated by the origin server. Origin servers should leave the 0 default
	 * value. Only caches are expected to set this property.<br>
	 * <br>
	...
	/**
	 * Returns the modifiable set of methods allowed on the requested resource. This
	 * property only has to be updated when a status CLIENT_ERROR_METHOD_NOT_ALLOWED
	 * is set. Creates a new instance if no one has been set.<br>
	 * <br>
	 * Note that when used with HTTP connectors, this property maps to the "Allow"
	 * header.
	 * 
	 ...
	 * entity may vary. If some server-side content negotiation is done, this set
	 * should be properly updated, other it can be left empty. Creates a new
	 * instance if no one has been set.<br>
	 * <br>
	 * Note that when used with HTTP connectors, this property maps to the "Vary"
	 * header.
	 * 
	...
	/**
	 * Returns the location reference. This is the reference that the client should
	 * follow for redirections or resource creations.<br>
	 * <br>
	 * Note that when used with HTTP connectors, this property maps to the
	 * "Location" header.
	 * 
	...
	/**
	 * Returns the server-specific information. Creates a new instance if no one has
	 * been set.
	 * 
	 ...
	 * "Access-Control-Allow-Credentials" header.
	 * 
	 * @param accessControlAllowCredentials True if the requested resource allows
	 *                                      credential.
	 */
	 ...
	 * preflight result cache.<br>
	 * Note that when used with HTTP connectors, this property maps to the
	 * "Access-Control-Max-Age" header.<br>
	 * In case of negative value, the header is not set.
	 * 
	 * @param accessControlMaxAge How long the results of a preflight request can be
	 ...
	 * when used as part of a response to a preflight CORS request.<br>
	 * Note that when used with HTTP connectors, this property maps to the
	 * "Access-Control-Allow-Headers" header.
	 * 
	 * @param accessControlAllowHeaders The set of headers allowed by the actual
	 *                                  request on the current resource.
	 */
	 ...
	 * "Access-Control-Allow-Methods" header.
	 * 
	 * @param accessControlAllowMethods The set of methods allowed by the actual
	 *                                  request on the current resource.
	 */
	 ...
	 * "Access-Control-Expose-Headers" header.
	 * 
	 * @param accessControlExposeHeaders The set of headers an origin server allows
	 ...
	 * set must be thread-safe (use {@link CopyOnWriteArraySet} for example.<br>
	 * <br>
	 * Note that when used with HTTP connectors, this property maps to the "Allow"
	 * header.
	 * 
	 ...
	 * "Authentication-Info" header.
	 * 
	 * @param authenticationInfo The data returned by the server in response to
	 ...
	 * client. Note that when used with HTTP connectors, this property maps to the
	 * "WWW-Authenticate" header. This method clears the current list and adds all
	 * entries in the parameter list.
	 * 
	 * @param challengeRequests A list of authentication requests sent by an origin
	 *                          server to a client.
	 */
	...
	/**
	 * Sets the modifiable series of cookie settings provided by the server. Note
	 * that when used with HTTP connectors, this property maps to the "Set-Cookie"
	 * and "Set-Cookie2" headers. This method clears the current series and adds all
	 * entries in the parameter series.
	 * 
	...
	/**
	 * Sets the set of dimensions on which the response entity may vary. Note that
	 * when used with HTTP connectors, this property maps to the "Vary" header. This
	 * method clears the current set and adds all entries in the parameter set.
	 * 
	 ...
	 * creations. Note that when used with HTTP connectors, this property maps to
	 * the "Location" header.
	 * 
	 ...
	 * {@link Request#getResourceRef()} and {@link Reference#getBaseRef()}.<br>
	 * <br>
	 * Note that when used with HTTP connectors, this property maps to the
	 * "Location" header.
	 * 
	 ...
	 * 
	 * @param proxyChallengeRequests A list of authentication requests sent by a
	 *                               proxy to a client.
	 */
