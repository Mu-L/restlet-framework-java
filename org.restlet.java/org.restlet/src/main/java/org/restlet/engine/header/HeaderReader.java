...
public class HeaderReader<V> {
		...
		if (next == '(') {
			...
			while (result == null) {
				...
				if (isCommentText(next)) {
					...
					throw new IOException(
							"Invalid character \"" + next + "\" detected in comment. Please check your value");
				}
			}
		} else {
		...
		if (isDoubleQuote(next)) {
			...
			while (result == null) {
				...
				if (isQuotedText(next)) {
					...
					throw new IOException(
							"Invalid character \"" + next + "\" detected in quoted string. Please check your value");
				}
			}
		} else {
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.w3.org/Protocols/rfc2616/rfc2616-sec2.html#sec2">HTTP
	 *      parsing rule</a>
	 * 
