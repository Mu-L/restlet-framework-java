...
public class ServerAdapter extends Adapter {
	 ...
	 * call. The default implementation first invokes the "addResponseHeaders" then
	 * asks the "htppCall" to send the response back to the client.
	 * 
	...
	public void commit(HttpResponse response) {
		try {
			if ((response.getRequest().getMethod() != null) && response.getRequest().getMethod().equals(Method.HEAD)) {
				...
				addEntityHeaders(response);
				getLogger()
						.warning("A response with a 200 (Ok) status should have an entity. Make sure that resource \""
