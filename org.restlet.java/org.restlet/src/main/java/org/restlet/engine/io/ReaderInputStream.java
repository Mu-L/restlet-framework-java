...
public class ReaderInputStream extends InputStream {
	...
	public ReaderInputStream(Reader reader, CharacterSet characterSet) throws IOException {
		...
		this.charsetEncoder = (characterSet == null) ? CharacterSet.ISO_8859_1.toCharset().newEncoder()
				: characterSet.toCharset().newEncoder();
	...
	public int read(byte[] b, int off, int len) throws IOException {
		...
		while (iterate) {
			...
			iterate = (len > 0)
					&& (!this.endReached || this.byteBuffer.hasRemaining() || this.charBuffer.hasRemaining());
		}
