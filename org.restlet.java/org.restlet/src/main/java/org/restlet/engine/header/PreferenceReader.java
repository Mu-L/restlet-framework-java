...
public class PreferenceReader<T extends Metadata> extends HeaderReader<Preference<T>> {
	 ...
	 * 
	 * @param acceptPatchHeader The header to parse.
	 * @param clientInfo        The client info to update.
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public static void addPatches(String acceptPatchHeader, ClientInfo clientInfo) {
		if (acceptPatchHeader != null) {
			PreferenceReader pr = new PreferenceReader(PreferenceReader.TYPE_PATCH, acceptPatchHeader);
	...
	/**
	 * Extract the media parameters. Only leave as the quality parameter if found.
