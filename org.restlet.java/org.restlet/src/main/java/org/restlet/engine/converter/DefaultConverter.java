...
public class DefaultConverter extends ConverterHelper {
	...
	public float score(Object source, Variant target, Resource resource) {
		...
		if (source instanceof String) {
			...
			if (target != null) {
				if (ObjectRepresentation.VARIANT_OBJECT_BINARY_SUPPORTED
						&& MediaType.APPLICATION_JAVA_OBJECT.equals(target.getMediaType())) {
					result = 1.0F;
				} else if (ObjectRepresentation.VARIANT_OBJECT_BINARY_SUPPORTED
						&& MediaType.APPLICATION_JAVA_OBJECT.isCompatible(target.getMediaType())) {
					result = 0.6F;
				} else if (ObjectRepresentation.VARIANT_OBJECT_XML_SUPPORTED
						&& MediaType.APPLICATION_JAVA_OBJECT_XML.equals(target.getMediaType())) {
					result = 1.0F;
				} else if (ObjectRepresentation.VARIANT_OBJECT_XML_SUPPORTED
						&& MediaType.APPLICATION_JAVA_OBJECT_XML.isCompatible(target.getMediaType())) {
					result = 0.6F;
				}
			} else if (ObjectRepresentation.VARIANT_OBJECT_BINARY_SUPPORTED) {
				result = 0.5F;
	...
	public <T> float score(Representation source, Class<T> target, Resource resource) {
		...
		if (target != null) {
			if (target.isAssignableFrom(source.getClass())) {
				...
				if (ObjectRepresentation.VARIANT_OBJECT_BINARY_SUPPORTED
						&& MediaType.APPLICATION_JAVA_OBJECT.equals(source.getMediaType())) {
					result = 1.0F;
				} else if (ObjectRepresentation.VARIANT_OBJECT_BINARY_SUPPORTED
						&& MediaType.APPLICATION_JAVA_OBJECT.isCompatible(source.getMediaType())) {
					result = 0.6F;
				} else if (ObjectRepresentation.VARIANT_OBJECT_XML_SUPPORTED
						&& MediaType.APPLICATION_JAVA_OBJECT_XML.equals(source.getMediaType())) {
					result = 1.0F;
				} else if (ObjectRepresentation.VARIANT_OBJECT_XML_SUPPORTED
						&& MediaType.APPLICATION_JAVA_OBJECT_XML.isCompatible(source.getMediaType())) {
					result = 0.6F;
				} else {
					result = 0.5F;
				}
	...
	public Representation toRepresentation(Object source, Variant target, Resource resource) throws IOException {
		...
		if (source instanceof String) {
			result = new StringRepresentation((String) source,
					MediaType.getMostSpecific(target.getMediaType(), MediaType.TEXT_PLAIN));
		} else if (source instanceof File) {
