...
public class Engine {
	...
	/**
	 * Creates a new standalone thread with local Restlet thread variable properly
	 * set.
	 * 
	...
	public Engine(boolean discoverHelpers) {
		...
		if (discoverHelpers) {
			try {
				...
				Context.getCurrentLogger().log(Level.WARNING, "An error occurred while discovering the engine helpers.",
						e);
			}
		}
	}
	...
	public org.restlet.engine.connector.ConnectorHelper<Client> createHelper(Client client, String helperClass) {
					...
					if ((helperClass == null) || connector.getClass().getCanonicalName().equals(helperClass)) {
						try {
							...
							Context.getCurrentLogger().log(Level.SEVERE,
									"Exception during the instantiation of the client connector.", e);
						}
					}
				}
			}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public org.restlet.engine.connector.ConnectorHelper<org.restlet.Server> createHelper(org.restlet.Server server,
			String helperClass) {
	 ...
	 */
	public org.restlet.engine.security.AuthenticatorHelper findHelper(ChallengeScheme challengeScheme,
	...
	public void registerDefaultConverters() {
		...
		getRegisteredConverters().add(new org.restlet.engine.converter.StatusInfoHtmlConverter());
	}
	...
	public void registerHelpers(ClassLoader classLoader, java.net.URL configUrl, List<?> helpers,
		...
		try {
			...
			try {
				reader = new java.io.BufferedReader(new InputStreamReader(configUrl.openStream(), "utf-8"),
						IoUtils.BUFFER_SIZE);
				...
				Context.getCurrentLogger().log(Level.SEVERE,
						"Unable to read the provider descriptor: " + configUrl.toString());
			} finally {
				if (reader != null) {
					reader.close();
				}
	 ...
	 * {@link java.net.URL#openConnection()} or {@link java.net.URL#openStream()}
	 * methods are invoked.
	 * <p>
					...
					protected java.net.URLConnection openConnection(java.net.URL url) throws IOException {
						return new java.net.URLConnection(url) {
							...
							public InputStream getInputStream() throws IOException {
								...
								if (context != null) {
									Response response = context.getClientDispatcher()
	 ...
	 */
	public void setRegisteredClients(List<org.restlet.engine.connector.ConnectorHelper<Client>> registeredClients) {
	 ...
	 */
	public void setRegisteredProtocols(List<org.restlet.engine.connector.ProtocolHelper> registeredProtocols) {
