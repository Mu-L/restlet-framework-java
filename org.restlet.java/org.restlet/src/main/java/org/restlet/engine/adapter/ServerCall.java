...
public abstract class ServerCall extends Call {
	...
	public ServerCall(Server server) {
		this((server == null) ? null : server.getAddress(), (server == null) ? 0 : server.getPort());
	}
	 ...
	 *
	 * @return The SSL session ID, as a byte array, if available and accessible in
	 *         that format.
	 */
	 ...
	 * optional entity and send them over the network. The default implementation
	 * only writes the response entity on the response stream or channel. Subclasses
	 * will probably also copy the response headers and status.
	 *
	...
	/**
	 * Effectively writes the response body. The entity to write is guaranteed to be
	 * non null. Attempts to write the entity on the response channel or response
	 * stream by default.
	 *
	 * @param entity               The representation to write as entity of the
	 *                             body.
	 * @param responseEntityStream The response entity stream or null if a channel
	 *                             is used.
	 * @throws IOException
	 */
