...
public abstract class EntityClientHelper extends LocalClientHelper {
	 ...
	 * @param encodedParentDirPath The encoded path of the parent directory of the
	 *                             requested resource.
	 ...
	 * percent-encoded name is given. Tries to match the longest common part of both
	 * encoded entity name and decoded variant name.
	 * 
	 * @param encodedEntityName        the percent-encoded name of the initial
	 *                                 resource
	 * @param decodedVariantEntityName the percent-decoded entity name of a variant
		...
		if (found) {
				...
				if (entities != null) {
					...
					for (Entity entry : entities) {
						if (baseName.equals(entry.getBaseName())) {
							rl.add(createReference(scheme, encodedParentDirectoryURI, encodedEntityName,
									entry.getName()));
						}
					}
			...
			if (entity.exists()) {
				...
				if (uniqueVariant != null) {
					// Return the file content
					output = uniqueVariant.getRepresentation(getMetadataService().getDefaultMediaType(),
							getTimeToLive());
					output.setLocationRef(request.getResourceRef());
					Entity.updateMetadata(entity.getName(), output, true, getMetadataService());
				}
