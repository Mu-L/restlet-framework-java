...
public class Validator extends Filter {
	 ...
	 * parses the template variable, adjust the base reference, then extracts the
	 * attributes from form parameters (query, cookies, entity) and finally tries to
	 * validate the variables as indicated by the
		...
		if (this.validations != null) {
			for (ValidateInfo validate : getValidations()) {
				if (validate.required && !request.getAttributes().containsKey(validate.attribute)) {
					response.setStatus(Status.CLIENT_ERROR_BAD_REQUEST, "Unable to find the \"" + validate.attribute
					...
					if ((value != null) && !Pattern.matches(validate.format, value.toString())) {
						response.setStatus(Status.CLIENT_ERROR_BAD_REQUEST,
								"Unable to validate the value of the \"" + validate.attribute
										+ "\" attribute. The expected format is: " + validate.format
	 ...
	 * then a response status CLIENT_ERROR_BAD_REQUEST is returned with the proper
	 * status description.
	 * 
	 * @param attribute Name of the attribute to look for.
	 * @param required  Indicates if the attribute presence is required.
	 ...
	 * response status CLIENT_ERROR_BAD_REQUEST is returned with the proper status
	 * description.
	 * 
	 * @param attribute Name of the attribute to look for.
	 * @param format    Format of the attribute value, using Regex pattern syntax.
	...
	/**
	 * Checks the request attributes for presence only. If the check fails, then a
	 * response status CLIENT_ERROR_BAD_REQUEST is returned with the proper status
	 * description.
	 * 
	 * @param attribute Name of the attribute to look for.
