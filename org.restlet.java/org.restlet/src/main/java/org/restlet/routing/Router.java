...
public class Router extends Restlet {
	...
	/**
	 * Each call is routed to the first route if the required score is reached. If
	 * the required score is not reached, then the route is skipped and the next one
	 * is considered. See {@link RouteList#getFirst(Request, Response, float)}
	 * method for implementation details.
	 */
	 ...
	 * previous one is considered. See
	 * {@link RouteList#getLast(Request, Response, float)} method for implementation
	 * details.
	 */
	 ...
	 * reached. The next route is relative to the previous call routed (round robin
	 * mode). If the required score is not reached, then the route is skipped and
	 * the next one is considered. If the last route is reached, the first route
	 * will be considered. See {@link RouteList#getNext(Request, Response, float)}
	 * method for implementation details.
	 */
	 ...
	 * result in non-uniform distribution of calls. See
	...
	/**
	 * The default setting for whether the routing should be done on URIs with or
	 * without taking into account query string.
	 */
	...
	/**
	 * The maximum number of attempts if no attachment could be matched on the first
	 * attempt.
	 */
	...
	/**
	 * Constructor. Note that usage of this constructor is not recommended as the
	 * Router won't have a proper context set. In general you will prefer to use the
	 * other constructor and pass it the parent application's context or eventually
	 * the parent component's context if you don't use applications.
	 */
	public Router() {
		this(null);
	}
	 ...
	 * route using the matching mode returned by {@link #getMatchingMode(Restlet)}
	 ...
	 * A new route using the matching mode returned by
	 * {@link #getMatchingMode(Restlet)} will be added routing to the target when
	 * calls with a URI matching the pattern will be received.
	 * 
	 * @param pathTemplate The URI path template that must match the relative part
	 *                     of the resource URI.
	...
	/**
	 * Attaches a target Resource class to this router based on a given URI pattern.
	 * A new route will be added routing to the target when calls with a URI
	...
	/**
	 * Attaches a target Restlet to this router based on a given URI pattern. A new
	 * route using the matching mode returned by {@link #getMatchingMode(Restlet)}
	 * will be added routing to the target when calls with a URI matching the
	 * pattern will be received.
	 * 
	 * @param pathTemplate The URI path template that must match the relative part
	 *                     of the resource URI.
	 * @param target       The target Restlet to attach.
	...
	/**
	 * Attaches a target Restlet to this router based on a given URI pattern. A new
	 * route will be added routing to the target when calls with a URI matching the
	 * pattern will be received.
	 * 
	 * @param pathTemplate The URI path template that must match the relative part
	 *                     of the resource URI.
	...
	/**
	 * Attaches a Resource class to this router as the default target to invoke when
	 * no route matches. It actually sets a default route that scores all calls to
	 * 1.0.
	 * 
	 ...
	 * route matches. It actually sets a default route that scores all calls to 1.0.
	 * 
	...
	/**
	 * Creates a new route for the given URI pattern, target and matching mode. The
	 * route will match the URI query string depending on the result of
	 ...
	 * Restlet are removed from the list of routes and the default route is set to
	 * null.
	 * 
	 ...
	 * typically the selected {@link Route}. By default, it just invokes the next
	 * Restlet.
	 * 
	 ...
	 * combination of the {@link #MODE_CUSTOM} option. The default implementation
	 * (to be overridden), returns null.
	 * 
	 ...
	 * By default it returns {@link Template#MODE_EQUALS}.
	 * 
	 ...
	 * with or without taking into account query string. By default, it returns
	 * false.
	 * 
	 * @return the default setting for whether the routing should be done on URIs
	 ...
	 * {@link Template#MODE_STARTS_WITH} to allow further routing by those objects.
	 * If the target is an instance of {@link Filter}, then it returns the matching
	 * mode for the {@link Filter#getNext()} Restlet recursively.
	 * 
	 ...
	 * the first attempt. This is useful when the attachment scoring is dynamic and
	 * therefore could change on a retry. The default value is set to 1.
	 * 
	 * @return The maximum number of attempts if no attachment could be matched on
	 *         the first attempt.
	 */
	...
	/**
	 * Returns the minimum score required to have a match. By default, it returns
	 * {@code 0.5}.
	 * 
	...
	/**
	 * Returns the delay in milliseconds before a new attempt is made. The default
	 * value is {@code 500}.
	 * 
	...
	/**
	 * Returns the modifiable list of routes. Creates a new instance if no one has
	 * been set.
	 * 
	...
	public TemplateRoute redirectSeeOther(String pathTemplate, String targetUri) {
		return attach(pathTemplate, new Redirector(getContext(), targetUri, Redirector.MODE_CLIENT_SEE_OTHER));
	}
	...
	public TemplateRoute redirectTemporary(String pathTemplate, String targetUri) {
		return attach(pathTemplate, new Redirector(getContext(), targetUri, Redirector.MODE_CLIENT_TEMPORARY));
	}

	/**
	 * Sets the default matching mode to use when selecting routes based on URIs. By
	 * default it is set to {@link Template#MODE_EQUALS}.
	 * 
	 ...
	 * or without taking into account query string. By default, it is set to false.
	 * 
	 * @param defaultMatchingQuery The default setting for whether the routing
	 *                             should be done on URIs with or without taking
	 *                             into account query string.
	 * 
	 */
	 ...
	 * first attempt. This is useful when the attachment scoring is dynamic and
	 * therefore could change on a retry.
	 * 
	...
	/**
	 * Sets the delay in milliseconds before a new attempt is made. By default, it
	 * is set to {@code 500}.
	 * 
	...
	/**
	 * Sets the routing mode. By default, it is set to the {@link #MODE_FIRST_MATCH}
	 * mode.
	 * 
