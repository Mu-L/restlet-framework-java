...
public abstract class Restlet implements Uniform {
	...
	/**
	 * Constructor with the Restlet's context which can be the parent's application
	 * context, but shouldn't be the parent Component's context for security
	 * reasons.
	 * 
	...
	/**
	 * Creates a new finder instance based on the "targetClass" property. If none is
	 * define, the {@link Application#createFinder(Class)} method is invoked if
	 ...
	 * parameter.
	 * 
	 * @param resourceClass The target {@link org.restlet.resource.ServerResource}
	 *                      class to find.
	 * @return The new finder instance.
	 * @see org.restlet.resource.Finder#createFinder(Class, Class, Context, Logger)
	 */
	public org.restlet.resource.Finder createFinder(
		...
		if (getFinderClass() != null) {
			result = org.restlet.resource.Finder.createFinder(resourceClass, getFinderClass(), getContext(),
					getLogger());
		} else if ((getApplication() != null) && (getApplication() != this)) {
			result = getApplication().createFinder(resourceClass);
		} else {
			result = org.restlet.resource.Finder.createFinder(resourceClass, org.restlet.resource.Finder.class,
	...
	/**
	 * Returns the finder class used to instantiate resource classes. By default, it
	 * returns the {@link org.restlet.resource.Finder} class. This property is
	 * leveraged by {@link Application#setOutboundRoot(Class)} and
	...
	/**
	 * Handles a call. The default behavior is to initialize the Restlet by setting
	 * the current context using the {@link Context#setCurrent(Context)} method and
	...
	/**
	 * Starts the Restlet. By default its only sets "started" internal property to
	 * true.
	 *
	 * WARNING: this method must be called at the end of the starting process by
	 * subclasses otherwise concurrent threads could enter into the call handling
	 * logic too early.
	 * 
	 * @throws Exception
	 */
	...
	/**
	 * Stops the Restlet. By default its only sets "started" internal property to
	 * false.
	 * 
	 * WARNING: this method must be called at the beginning of the stopping process
	 * by subclasses otherwise concurrent threads could continue to (improperly)
	 * handle calls.
	 * 
	 * @throws Exception
	 */
