...
public class Context {
	...
	/**
	 * Returns the context associated to the current {@link Restlet}. The context
	 * can be the one of a {@link Component}, an {@link Application}, a
	 ...
	 * with empty or null properties, except the client and server dispatchers that
	 ...
	 * information relative to the context. This is a convenient means to provide
	 * common objects to all the Restlets and Resources composing an
	 * Application.<br>
	 * <br>
	 * 
	 * In addition, this map is a shared space between the developer and the Restlet
	 * implementation. For this purpose, all attribute names starting with
	...
	/**
	 * Returns a request dispatcher to available client connectors. When you ask the
	 * dispatcher to handle a request, it will automatically select the appropriate
	 * client connector for your request, based on the request.protocol property or
	 * on the resource URI's scheme. This call is blocking and will return an
	...
	/**
	 * Returns a request dispatcher to component's virtual hosts. This is useful for
	 * application that want to optimize calls to other applications hosted in the
	 * same component or to the application itself.<br>
	 * <br>
	 * The processing is the same as what would have been done if the request came
	 * from one of the component's server connectors. It first must match one of the
	 * registered virtual hosts. Then it can be routed to one of the attached
	 * Restlets, typically an Application.<br>
	 * <br>
