 ...
 * <br>
 * The default access log format follows the
 * <a href="http://www.w3.org/TR/WD-logfile.html"> W3C Extended Log File
 * Format</a> with the following fields used: <br>
 * <ol>
 ...
 * </ol>
 * <br>
 * If you use <a href="https://github.com/fabianbuechler/analog">Analog</a> to
 * generate your log reports, and if you use the default log format, then you
 * can simply specify this string as a value of the LOGFORMAT command:
 ...
 * 
 * @see <a href=
 *      "https://docs.oracle.com/javase/8/docs/api/java/util/logging/package-summary.html">java.util.logging</a>
...
public class LogService extends Service {
	...
	protected String getDefaultResponseLogMessage(Response response, int duration) {
		...
		if (!response.isEntityAvailable() || Status.REDIRECTION_NOT_MODIFIED.equals(response.getStatus())
				|| Status.SUCCESS_NO_CONTENT.equals(response.getStatus()) || Method.HEAD.equals(request.getMethod())) {
			sb.append('0');
		} else {
	 ...
	 * default, meaning the all requests are loggable, independant of their target
	...
	/**
	 * Returns the name of the JDK's logger to use when logging access calls. The
	 * default name will follow this pattern: "org.restlet.MyComponent.LogService",
	 * where "MyComponent" will correspond to the simple class name of your
	 * component subclass or to the base "Component" class.
	 * 
	...
	/**
	 * Format an access log entry. If the log template property isn't provided, then
	 * a default IIS like format is used.
	 * 
	 * @param response The response to log.
	...
	/**
	 * Sets the format to use when logging responses. The default format matches the
	 * one of IIS 6.
	 * 
