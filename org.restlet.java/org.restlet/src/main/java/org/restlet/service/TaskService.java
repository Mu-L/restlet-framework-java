...
public class TaskService extends Service implements ScheduledExecutorService {
	 ...
	 * will ensure that call to static methods like {@link Application#getCurrent()}
	 * still work.
	 * 
						...
						if (runnable instanceof ContextualRunnable) {
							...
							try {
								...
								Thread.currentThread()
										.setContextClassLoader(((ContextualRunnable) runnable).getContextClassLoader());
								runnable.run();
							} finally {
	...
	/**
	 * Allow {@link #shutdown()} and {@link #shutdownNow()} methods to effectively
	 * shutdown the wrapped executor service.
	 */
	 ...
	 * 
	 * @param enabled True if the service has been enabled.
	 * @param daemon  True if the threads are created as daemon threads.
	 */
	 ...
	 * 
	 * @param enabled      True if the service has been enabled.
	 * @param corePoolSize The core pool size defining the maximum number of
	 *                     threads.
	 */
	...
	/**
	 * Creates a new JDK executor service that will be wrapped. By default it calls
	 * {@link Executors#newCachedThreadPool(ThreadFactory)}, passing the result of
	 * {@link #createThreadFactory()} as a parameter.
	 * 
	 * @param corePoolSize The core pool size defining the maximum number of
	 *                     threads.
	 ...
	 * and results when all complete.<br>
	 * <br>
	 ...
	 * the generic information from this method. You can check the
	 ...
	 * first. Future.isDone() is true for each element of the returned list. Upon
	 * return, tasks that have not completed are canceled. Note that a completed
	 ...
	 * while this operation is in progress.<br>
	 * <br>
	 * Due to a breaking change between Java SE versions 5 and 6, and in order to
	 * maintain compatibility both at the source and binary level, we have removed
	 * the generic information from this method. You can check the
	...
	/**
	 * Indicates if the {@link #shutdown()} and {@link #shutdownNow()} methods are
	 ...
	 * Indicates whether or not the threads are daemon threads. True by default.
	 * 
	 * @param daemon True if the threads are daemon threads.
	 */
