...
public class ConverterService extends Service {
	 ...
	 * obtain a modified one. The patch must have a recognized media type in order
	 * for the {@link ConverterService} to be able to process it.
	 * 
	 * @param initial The initial representation on which the patch must be applied.
	 * @param patch   The patch representation to apply.
	 ...
	 * 
	 * @param initial  The initial representation.
	 * @param modified The modified representation.
	 * @return The patch representation able to convert the initial representation
	 *         into the modified representation.
	 * @throws IOException
	 */
	 ...
	 * types.
	 * 
	 * @param representationType The representation media type or null for all
	 ...
	 * obtain the initial one. The patch must have a recognized media type in order
	 * for the {@link ConverterService} to be able to process it.
	 * 
	 * @param modified The modified representation from which the patch must be
	 *                 reverted.
	 * @param patch    The patch representation to revert.
			...
			if (ch != null) {
				if (loggable && Context.getCurrentLogger().isLoggable(Level.FINE)) {
					Context.getCurrentLogger()
							.fine("The following converter was selected for the " + source + " representation: " + ch);
				}
				...
				if (loggable) {
					Context.getCurrentLogger()
							.warning("Unable to find a converter for this representation : " + source);
				}
			}
		}
	...
	/**
	 * Converts a regular Java object into a Representation. The converter will use
	 * the preferred variant of the selected converter.
	 * 
	...
	public Representation toRepresentation(Object source, Variant target, Resource resource) throws IOException {
		...
		if (ch != null) {
			if (loggable && Context.getCurrentLogger().isLoggable(Level.FINE)) {
				Context.getCurrentLogger().fine("Converter selected for " + source.getClass().getSimpleName() + ": "
			...
			if (target == null) {
				...
				if ((variants != null) && !variants.isEmpty()) {
					if (resource != null) {
						target = resource.getConnegService().getPreferredVariant(variants, resource.getRequest(),
	...
	/**
	 * Updates the media type preferences with available conversion capabilities for
