...
public abstract class Resource {
	...
	/**
	 * Invoked when a {@link Throwable} is caught during initialization, handling or
	 * releasing.
	 * 
	 ...
	 * resource. By default it does nothing.
	 * 
	...
	/**
	 * Clean-up method that can be overridden in order to release the state of the
	 * resource. By default it does nothing.
	 * 
	 * @see #release()
	 */
	 ...
	 * The result can vary based on the client's user agent, authentication and
	...
	/**
	 * Returns the parent application. If it wasn't set, it attempts to retrieve the
	 * current one via {@link org.restlet.Application#getCurrent()} if it exists, or
	 * instantiates a new one as a last resort.
	 * 
	 ...
	 * response attributes maps. This is typically used for variables that are
	 * declared in the URI template used to route the call to this resource.
	 * 
	 ...
	 * new instance if no one has been set.
	 * 
	 ...
	 * a client. If none is available, an empty list is returned.
	 * 
	...
	/**
	 * Returns the resource reference's optional query. Note that modifications to
	 * the returned {@link Form} object aren't reported to the underlying reference.
	 * 
	 ...
	 * instance. It the calls the {@link #doInit()} method that can be overridden.
	 * 
	 ...
	 * 
	 * @param source The object to convert.
	 * @param target The target representation media type.
	 * @return The wrapper representation.
	 * @throws IOException
	 */
	...
	public String toString() {
		return (getRequest() == null ? "" : getRequest().toString())
				+ (getResponse() == null ? "" : " => " + getResponse().toString());
	}
}
