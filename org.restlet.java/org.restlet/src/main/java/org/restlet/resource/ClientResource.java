...
public class ClientResource extends Resource {
	...
	/**
	 * Creates a client resource that proxy calls to the given Java interface into
	 * Restlet method calls. It basically creates a new instance of
	 * {@link ClientResource} and invokes the {@link #wrap(Class)} method.
	 * 
	 * @param <T>               The proxified interface.
	...
	/**
	 * Indicates if transient or unknown size request entities should be buffered
	 * before being sent.
	 */
	...
	/**
	 * Adds a parameter to the query component. The name and value are automatically
	...
	/**
	 * Adds a segment at the end of the path. If the current path doesn't end with a
	 * slash character, one is inserted before the new segment value. The value is
	 * automatically encoded if necessary.
	 * 
	...
	/**
	 * Deletes the target resource and all its representations. If a success status
	 * is not returned, then a resource exception is thrown.
	 * 
	 * @return The optional response entity.
	 * @see <a href=
	 *      "http://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.7">HTTP
	 *      DELETE method</a>
	 */
	...
	/**
	 * By default, it throws a new resource exception. Call
	 * {@link #doError(org.restlet.data.Status, org.restlet.Request, org.restlet.Response)}.
	 *
	 * @param request  The associated request.
	 ...
	 * based on the client preferences. Note that the client preferences will be
	 * automatically adjusted, but only for this request. If you want to change them
	 * once for all, you can use the {@link #getClientInfo()} method.<br>
	 * <br>
	 ...
	 * @throws ResourceException
	 * @see <a href=
	 *      "http://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.3">HTTP GET
	 *      method</a>
	 */
	 ...
	 * preferences will be automatically adjusted, but only for this request. If you
	 * want to change them once for all, you can use the {@link #getClientInfo()}
	 * method.<br>
	 * <br>
	 * If a success status is not returned, then a resource exception is thrown.
	 * 
	 ...
	 * resource. The child resource is defined in the sense of hierarchical URIs. If
	 * the resource URI is not hierarchical, then an exception is thrown.
	 * 
	 ...
	 * into Restlet method calls. The child resource is defined in the sense of
	 * hierarchical URIs. If the resource URI is not hierarchical, then an exception
	 * is thrown.
	 *
	 * @param <T>               The proxified interface.
	 * @param relativeRef       The URI reference of the child resource relatively
	 *                          to the current resource seen as the parent resource.
	 * @param resourceInterface The annotated resource interface class to proxy.
	 * @return The proxy instance.
	 */
	 ...
	 *
	 * @param <T>               The proxified interface.
	 * @param relativeUri       The URI of the child resource relatively to the
	 *                          current resource seen as the parent resource.
	 * @param resourceInterface The annotated resource interface class to proxy.
	 * @return The proxy instance.
	 */
	 ...
	 * for a single call. Default value is 10.
	 * 
	 * @return The maximum number of redirections that can be automatically followed
	 *         for a single call.
	 */
	...
	/**
	 * Returns the callback invoked on response reception. If the value is not null,
	 * then the associated request will be executed asynchronously.
	 * 
	...
	/**
	 * Returns the parent resource. The parent resource is defined in the sense of
	 * hierarchical URIs. If the resource URI is not hierarchical, then an exception
	 * is thrown.
	 * 
	 ...
	 * into Restlet method calls. The parent resource is defined in the sense of
	 * hierarchical URIs. If the resource URI is not hierarchical, then an exception
	 * is thrown.
	 * 
	 * @param <T>               The proxified interface.
	...
	/**
	 * Returns the number of retry attempts before reporting an error. Default value
	 * is 2.
	 * 
	...
	/**
	 * Returns the delay in milliseconds between two retry attempts. Default value
	 * is 2 seconds.
	 * 
	 ...
	 * retrieved via {@link #getRequest()} and cloned and the response is set as the
	 * latest with {@link #setResponse(Response)}. If necessary the
	...
	protected <T> T handle(Method method, Object entity, Class<T> resultClass) throws ResourceException {
		...
		if (entity != null) {
			...
			try {
				...
				request.setEntity(toRepresentation(entity,
						getConnegService().getPreferredVariant(entityVariants, request, getMetadataService())));
			} catch (IOException e) {
				throw new ResourceException(e);
			}
		} else {
	 ...
	 * @param references   The references that caused a redirection to prevent
	 *                     infinite loops.
	 * @param retryAttempt The number of remaining attempts.
	 * @param next         The next handler handling the call.
	 */
	protected void handle(Request request, Response response, List<Reference> references, int retryAttempt,
			Uniform next) {
		if (next != null) {
			...
			if (isRetryOnError() && response.getStatus().isRecoverableError() && request.getMethod().isIdempotent()
	...
	/**
	 * Represents the resource using content negotiation to select the best variant
	 * based on the client preferences. This method is identical to {@link #get()}
	 * but doesn't return the actual content of the representation, only its
	 * metadata.<br>
	 * <br>
	 * Note that the client preferences will be automatically adjusted, but only for
	 * this request. If you want to change them once for all, you can use the
	 ...
	 * @throws ResourceException
	 * @see <a href=
	 *      "http://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.4">HTTP HEAD
	 *      method</a>
	 */
	...
	/**
	 * Represents the resource using a given media type. This method is identical to
	 * {@link #get(MediaType)} but doesn't return the actual content of the
	...
	/**
	 * Indicates if transient or unknown size response entities should be buffered
	 * after being received. This is useful to increase the chance of being able to
	 * resubmit a failed request due to network error, or to prevent chunked
	 * encoding from being used an HTTP connector.
	 * 
	 * @return True if transient response entities should be buffered after being
	 *         received.
	 */
	...
	/**
	 * Indicates if transient or unknown size response entities should be buffered
	 * after being received. This is useful to be able to systematically reuse and
	 * process a response entity several times after retrieval.
	 * 
	 * @return True if transient response entities should be buffered after being
	 *         received.
	 */
	...
	/**
	 * Indicates if idempotent requests should be retried on error. Default value is
	 * true.
	 * 
	...
	/**
	 * Describes the resource using content negotiation to select the best variant
	 * based on the client preferences. If a success status is not returned, then a
	 * resource exception is thrown.
	 * 
	 * @return The best description.
	 * @throws ResourceException
	 * @see <a href=
	 *      "http://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.2">HTTP
	 *      OPTIONS method</a>
	 */
	...
	/**
	 * Describes the resource using a given media type. If a success status is not
	 ...
	 * serializes the object using the {@link org.restlet.service.ConverterService}.
	 * 
	 * @param entity The object entity containing the patch.
	 ...
	 * @see <a href="https://tools.ietf.org/html/rfc5789">HTTP PATCH method</a>
	 */
	public <T> T patch(Object entity, Class<T> resultClass) throws ResourceException {
	 ...
	 * @see <a href="https://tools.ietf.org/html/rfc5789">HTTP PATCH method</a>
	 */
	public Representation patch(Object entity, MediaType mediaType) throws ResourceException {
		try {
	...
	/**
	 * Patches a resource with the given representation as delta state. If a success
	 * status is not returned, then a resource exception is thrown.
	 * 
	 * @param entity The request entity containing the patch.
	 * @return The optional result entity.
	 * @throws ResourceException
	 * @see <a href="https://tools.ietf.org/html/rfc5789">HTTP PATCH method</a>
	 */
	 ...
	 * @throws ResourceException
	 * @see <a href=
	 *      "http://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.5">HTTP POST
	 *      method</a>
	 */
	...
	/**
	 * Posts a representation. If a success status is not returned, then a resource
	 * exception is thrown.
	 * 
	 ...
	 * @throws ResourceException
	 * @see <a href=
	 *      "http://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.6">HTTP PUT
	 *      method</a>
	 */
	 ...
	 * be stored. If a success status is not returned, then a resource exception is
	 * thrown.
	 * 
	...
	/**
	 * Effectively redirects a client call. By default, it checks for infinite loops
	 * and unavailable entities, the references list is updated and the
	 ...
	 * @param next         The next handler handling the call.
	 */
	protected void redirect(Request request, Response response, List<Reference> references, int retryAttempt,
			Uniform next) {
	...
	/**
	 * Effectively retries a failed client call. By default, it sleeps before the
	 * retry attempt and increments the number of retries.
	 * 
	 * @param request      The request to send.
	 * @param response     The response to update.
	 * @param references   The references that caused a redirection to prevent
	 ...
	 * @param next         The next handler handling the call.
	 */
	protected void retry(Request request, Response response, List<Reference> references, int retryAttempt,
			Uniform next) {
		getLogger().log(Level.INFO, "A recoverable error was detected (" + response.getStatus().getCode()
	 ...
	 * 
	 * @param challengeResponse The authentication response sent by a client to an
	 *                          origin server.
	 ...
	 * for a single call.
	 * 
	 * @param maxRedirects The maximum number of redirections that can be
	 *                     automatically followed for a single call.
	 */
	 ...
	 * 
	 * In addition, this method will set the context of the next Restlet if it is
	 * null by passing a reference to its own context.
	 * 
	...
	/**
	 * Sets the callback invoked on response reception. If the value is not null,
	 * then the associated request will be executed asynchronously.
	 * 
	 ...
	 * 
	 * @param challengeResponse The proxy authentication response sent by a client
	 *                          to an origin server.
	 ...
	 * resolved as an absolute reference. Also, the context's base reference will be
	 * reset. Finally, the reference will be normalized to ensure a consistent
	...
	/**
	 * Sets the resource's reference using an URI string. Note that the URI can be
	 * either absolute or relative to the context's base reference.
	 * 
	 ...
	 * encoding from being used an HTTP connector.
	 * 
	 * @param requestEntityBuffering True if transient request entities should be
	 ...
	 * process a response entity several times after retrieval.
	 * 
	 * @param responseEntityBuffering True if transient response entities should be
	 *                                buffered after being received.
	 */
	...
	/**
	 * Sets the delay in milliseconds between two retry attempts. The default value
	 * is two seconds.
	 * 
	 ...
	 * Restlet method calls. Use the {@link org.restlet.engine.Engine} classloader
	 * in order to generate the proxy.
	 * 
	 * @param <T>
	 * @param resourceInterface The annotated resource interface class to proxy.
	 * @return The proxy instance.
	...
	public <T> T wrap(Class<? extends T> resourceInterface) {
		return wrap(resourceInterface, org.restlet.engine.Engine.getInstance().getClassLoader());
	}
	 ...
	 * @param <T>
	 * @param resourceInterface The annotated resource interface class to proxy.
	 * @param classLoader       The classloader used to instantiate the dynamic
	 *                          proxy.
	 * @return The proxy instance.
	 */
	@SuppressWarnings("unchecked")
	public <T> T wrap(Class<? extends T> resourceInterface, ClassLoader classLoader) {
		T result = null;

		// Create the client resource proxy
		java.lang.reflect.InvocationHandler h = new org.restlet.engine.resource.ClientInvocationHandler<T>(this,
				resourceInterface);
