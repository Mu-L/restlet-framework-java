...
public class Server extends Connector {
	...
	/**
	 * Returns the actual server port after it has started. If an ephemeral port is
	 * used it will be returned, otherwise the fixed port will be provided.
	 * 
	 ...
	 * The default value is '-1' if no ephemeral port is known. See
	...
	/**
	 * Sets the next Restlet as a Finder for a given resource class. When the call
	 * is delegated to the Finder instance, a new instance of the resource class
	 * will be created and will actually handle the request.
	 * 
	 ...
	 * retrieved once the server is started using the {@link #getEphemeralPort()}
	 * method.
	 * 
