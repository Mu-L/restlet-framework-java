...
public class SelectionRegistration {
	 ...
	 */
	public SelectionRegistration(int interestOperations, SelectionListener selectionListener,
	 ...
	 * @param wakeupListener     The wakeup listener that will be notified.
	 */
	public SelectionRegistration(SelectableChannel selectableChannel, int interestOperations,
			...
			if (Context.getCurrentLogger().isLoggable(Level.FINEST)) {
				Context.getCurrentLogger().log(Level.FINEST,
						"Calling thread about to block on the NIO selection registration. Timeout: "
								+ TimeUnit.MILLISECONDS.toMillis(IoUtils.TIMEOUT_MS) + " ms. Waiting: "
	...
	/**
	 * Resume interest in new listener notifications. This should be called after a
	 * {@link #suspend()} call.
	 */
	public void resume() {
	...
	public String toString() {
		return "Interest= " + getName(getInterestOperations()) + ", Ready=" + getName(getReadyOperations())
				+ ", Canceling=" + Boolean.toString(isCanceling());
	}
	...
	public void unblock() throws IOException {
		...
		try {
			...
			Context.getCurrentLogger().log(Level.WARNING, "Unable to unblock the waiting thread at the cyclic barrier",
					e);
	...
	/**
	 * Effectively updates the registration of the {@link #getSelectableChannel()}
	 * with the given {@link Selector} for the {@link #getInterestOperations()}
	 * operations.
	 * 
	 * @return The updated selection key or a new one if it was registered again.
	 */
			...
			if (isCanceling()) {
				...
				try {
					if (Context.getCurrentLogger().isLoggable(Level.FINEST)) {
						Context.getCurrentLogger().log(Level.FINEST,
								"Update key (old | new) : "
										+ SelectionRegistration.getName(this.selectionKey.interestOps()) + " | "
										+ SelectionRegistration.getName(getInterestOperations()));
					}

					this.selectionKey.interestOps(getInterestOperations());
				} catch (CancelledKeyException cke) {
					Context.getCurrentLogger().log(Level.FINE, "Unable to update a cancelled key, registering again",
							cke);
