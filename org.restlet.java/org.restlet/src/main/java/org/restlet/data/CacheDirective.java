...
public final class CacheDirective implements NamedValue<String> {
	...
	/**
	 * Creates a "max-age" directive. Indicates that the client is willing to accept
	 * a response whose age is no greater than the specified time in seconds. Unless
	 * "max-stale" directive is also included, the client is not willing to accept a
	 * stale response.<br>
	 * <br>
	 ...
	 * @see <a href=
	 *      "http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.9.3">HTTP
	 *      1.1 - Modifications of the Basic Expiration Mechanism</a>
	 * @see <a href=
	 *      "http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.9.4">HTTP
	...
	/**
	 * Creates a "max-stale" directive. Indicates that the client is willing to
	 * accept a response that has exceeded its expiration time by a given amount of
	 * time.<br>
	 * <br>
	 * Note that this directive can be used on requests only.
	 * 
	 ...
	 * accept a response whose freshness lifetime is no less than its current age
	 ...
	 * response to satisfy subsequent requests without successful revalidation with
	 ...
	 * @see <a href=
	 *      "http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.9.1">HTTP
	...
	/**
	 * Creates a "no-store" directive. Indicates that a cache must not release or
	 * retain any information about the call. This applies to both private and
	 * shared caches.<br>
	 * <br>
	 * Note that this directive can be used on requests or responses.
	 * 
	 ...
	 * @see <a href=
	 *      "http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.9.2">HTTP
	...
	/**
	 * Creates a "no-transform" directive. Indicates that a cache or intermediary
	 * proxy must not transform the response entity.<br>
	 * <br>
	 * Note that this directive can be used on requests or responses.
	 * 
	 * @return A new "no-transform" directive.
	 * @see <a href=
	 *      "http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.9.5">HTTP
	 ...
	 * requires revalidation of a cache entry on any subsequent use, except that it
	 * does not apply to non-shared user agent caches<br>
	 * <br>
	 * Note that this directive can be used on responses only.
	 * 
	...
	/**
	 * Creates a "public" directive. Indicates that the response may be cached by
	 * any cache, even if it would normally be non-cacheable or cacheable only
	 ...
	 * accept a response from a shared cache (but not a private cache) whose age is
	 * no greater than the specified time in seconds.<br>
	 * <br>
	 * Note that this directive can be used on responses only.
	 * 
