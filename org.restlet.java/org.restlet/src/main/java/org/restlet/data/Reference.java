...
public class Reference {
	...
	static {
		...
		for (int character = 0; character < 127; character++) {
			charValidityMap[character] = isReserved(character) || isUnreserved(character) || (character == '%')
					|| (character == '{') || (character == '}');
		}
	}
	...
	public static String encode(String toEncode, boolean queryString, CharacterSet characterSet) {
		...
		try {
			...
			Context.getCurrentLogger().log(Level.WARNING, "Unable to encode the string with the UTF-8 character set.",
					uee);
		}
	 ...
	 */
	public static String toString(String scheme, String hostName, Integer hostPort, String path, String query,
			String fragment) {
	 ...
	 * @param query        The optional query component for hierarchical
	 *                     identifiers.
	 * @param fragment     The optional fragment identifier.
	 ...
	 * Constructor from an {@link java.net.URI} instance.
	 * 
	 * @param baseUri The base {@link java.net.URI} instance.
	 * @param uri     The {@link java.net.URI} instance.
	 */
	...
	/**
	 * Constructor for a protocol and host name. Uses the default port for the given
	 * protocol.
	 * 
	 ...
	 * 
	 * @param baseRef      The base reference.
	 * @param relativePart The relative part component (most of the time it is the
	 *                     path component).
	 * @param query        The optional query component for hierarchical
	 *                     identifiers.
	 * @param fragment     The optional fragment identifier.
			...
			for (int i = 0; valid && (i < uriRef.length()); i++) {
				if (!isValid(uriRef.charAt(i))) {
					...
					Context.getCurrentLogger().fine("Invalid character detected in URI reference at index '" + i
							+ "': \"" + uriRef.charAt(i) + "\". It will be automatically encoded.");
					...
					valid = false;
					Context.getCurrentLogger().fine("Invalid percent encoding detected in URI reference at index '" + i
							+ "': \"" + uriRef.charAt(i) + "\". It will be automatically encoded.");
				}
			}
	...
	/**
	 * Returns the authority component for hierarchical identifiers. Includes the
	 * user info, host name and the host port number.<br>
	...
	/**
	 * Returns the optional extensions for hierarchical identifiers. An extensions
	...
	/**
	 * Returns the host identifier. Includes the scheme, the host name and the host
	 * port number.<br>
	 * Note that no URI decoding is done by this method.
	 * 
		...
		if (authority != null) {
			...
			if (index != -1) {
				try {
					...
					Context.getCurrentLogger().log(Level.WARNING, "Can't parse hostPort : [hostRef,requestUri]=["
	 ...
	 * @param decode        Indicates if the result should be decoded using the
	 *                      {@link #decode(String)} method.
	 * @param excludeMatrix True if the matrix parameters are dropped from the
	 *                      segments.
	 * @return The optionally decoded last segment.
	 * @see #getLastSegment()
	 */
	...
	/**
	 * Returns the parent reference of a hierarchical reference. The last slash of
	 * the path will be considered as the end of the parent path.
	 * 
	 ...
	 * reference. Note that the optional fragment is not returned by this method.
	 * Must be used with the following prerequisites:
	 * <ul>
	 ...
	 *                                  not absolute.
	 * @throws IllegalArgumentException If the reference is relative and not base
	...
	/**
	 * Sets the extensions for hierarchical identifiers. An extensions part starts
	 * after the first '.' character of the last path segment and ends with either
	 * the end of the segment of with the first ';' character (matrix start). It is
	 * a token similar to file extensions separated by '.' characters. The value can
	 * be ommited.<br>
	 * Note that no URI decoding is done by this method.
	 * 
	 ...
	 * 
	 * @param fragment The fragment identifier.
	 * @throws IllegalArgumentException if the fragment parameter contains the
	 *                                  fragment delimiter ('#').
	 */
	 ...
	 * @throws IllegalArgumentException If the identifier parameter contains the
	 *                                  fragment delimiter ('#').
	 */
	 ...
	 * one and adds a slash in front of the given last segmetn. <br>
	 * Note that no URI decoding is done by this method.
	 * 
	...
	/**
	 * Converts to a {@link java.net.URI} instance. Note that relative references
	 * are resolved before conversion using the {@link #getTargetRef()} method.
	 * 
	...
	/**
	 * Converts to a {@link java.net.URL} instance. Note that relative references
	 * are resolved before conversion using the {@link #getTargetRef()} method.
	 * 
