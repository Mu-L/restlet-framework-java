...
public class MemoryRealm extends Realm {
	...
	/**
	 * Verifier based on the default security model. It looks up users in the mapped
	 * organizations.
	 */
	 ...
	 */
	private void addGroups(User user, Set<Group> userGroups, Group currentGroup, List<Group> stack,
			boolean inheritOnly) {
	...
	/**
	 * Finds the set of groups where a given user is a member. Note that inheritable
	 * ancestors groups are also returned.
	 * 
	 * @param user The member user.
	...
	/**
	 * Sets the modifiable list of root groups. This method clears the current list
	...
	/**
	 * Sets the modifiable list of users. This method clears the current list and
	...
	/**
	 * Unmaps an element (user, group or organization) defined in a component from a
	 * role defined in the application.
	 * 
