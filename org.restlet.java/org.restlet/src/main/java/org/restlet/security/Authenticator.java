...
public abstract class Authenticator extends Filter {
	...
	/**
	 * Indicates if the authenticator is not required to succeed. In those cases,
	 * the attached Restlet is invoked. Note that authentication will be attempted
	 * independently of this property unless the client is already authenticated and
	 * the {@link #isMultiAuthenticating()} prevents multiple authentications.
	 */
	 ...
	 * 
	 * @param context             The context.
	 * @param multiAuthenticating Indicates if the authenticator should attempt to
	 *                            authenticate an already authenticated client.
	 * @param optional            Indicates if the authenticator is not required to
	 *                            succeed.
	 * @param enroler             The enroler to invoke upon successful
	 *                            authentication.
	 */
	...
	/**
	 * Invoked upon successful authentication. By default, it updates the request's
	 * clientInfo and challengeResponse "authenticated" properties, clears the
	 * existing challenge requests on the response, calls the enroler and finally
	 * returns {@link Filter#CONTINUE}.
	 * 
	 * @param request  The request sent.
	 * @param response The response to update.
	...
	protected int authenticated(Request request, Response response) {
		...
		if (loggable && request.getChallengeResponse() != null) {
			getLogger().log(Level.FINE,
					"The authentication succeeded for the identifer \"" + request.getChallengeResponse().getIdentifier()
	 ...
	 * {@link ClientInfo#isAuthenticated()} returns false. If the method is invoked
	 * and returns true, the {@link #authenticated(Request, Response)} is called.
	 * Otherwise, if {@link #isOptional()} returns true it continues to the next
	 * Restlet or if it returns false it calls the
	 ...
	 * subject with new principals. Typically new {@link Role} are added based on
	 * the available {@link User} instances available.
	 * 
	 ...
	 * {@link ClientInfo#isAuthenticated()} returns true. By default, it is set to
	 * true.
	 * 
	 * @return True if the authenticator should attempt to authenticate an already
	 ...
	 * true.
	 * 
	 * @param multiAuthenticating True if the authenticator should attempt to
	 *                            authenticate an already authenticated client.
	 */
